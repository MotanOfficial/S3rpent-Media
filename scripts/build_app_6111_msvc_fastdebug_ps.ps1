...
$DEFAULT_SHORT_BUILD_DIRNAME = "s3_fd_6111"
...
if ($env:S3_BUILD_DIR -and -not [string]::IsNullOrWhiteSpace($env:S3_BUILD_DIR)) {
    ...
    if (Test-Path $DEFAULT_SHORT_BUILD_ROOT) {
        ...
        $BUILD_DIR = Join-Path $PROJECT_DIR "build\fd"
    }
}
...
Write-Host "Setting up Qt 6.11.1 with MSVC..." -ForegroundColor Cyan
$qtEnv = "C:\Qt\6.11.1\msvc2022_64\bin\qtenv2.bat"
if (Test-Path $qtEnv) {
...
# Install libdatachannel for WebRTC P2P listen together feature
Write-Host "Installing libdatachannel for WebRTC P2P..." -ForegroundColor Cyan
& $VCPKG_EXE @("install", "libdatachannel:$VCPKG_TRIPLET")
if ($LASTEXITCODE -ne 0) {
    Write-Host "Failed to install libdatachannel via vcpkg." -ForegroundColor Red
    exit 1
}
...
Write-Host "Configuring project with CMake + Ninja (MSVC) for FASTDEBUG (RelWithDebInfo)..." -ForegroundColor Cyan
...
# Build
Write-Host "`nBuilding Qt project with Qt 6.11.1 (MSVC) - FASTDEBUG (RelWithDebInfo)..." -ForegroundColor Cyan
