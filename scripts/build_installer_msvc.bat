...
echo Copying MSVC release files...
...
REM Copy icon to package data (updated path)
if exist "%PROJECT_DIR%\resources\icons\icon.ico" (
    copy /Y "%PROJECT_DIR%\resources\icons\icon.ico" "%PACKAGE_DATA%\" >nul
    echo Copied application icon
) else if exist "%PROJECT_DIR%\icon.ico" (
    REM Fallback to root directory for backwards compatibility
    copy /Y "%PROJECT_DIR%\icon.ico" "%PACKAGE_DATA%\" >nul
    echo Copied application icon (from root)
)
...
echo.
echo ========================================
echo   MSVC Installer Created Successfully!
echo ========================================
echo.
